...
O projeto **basic**, básico, consiste em uma API REST desenvolvida com o framework Node.js/Express em TypeScript, banco de dados PostegreSQL, acessado através do ORM TypeORM e upload de arquivos através da biblioteca Multer.
...
```
  ...
  postgres=# create user basic_user with encrypted password '123456'
  postgres=# create database basic_db
  postgres=# grant all privileges on database basic_db to basic_user
  postgres=# exit
```
...
```
  ...
  [+] Running 3/3
   - Network network_basic     Created                                  0.0s
   - Container database_basic  Created                                  0.0s
   - Container redis_basic     Created                                  0.0s
  Attaching to database_basic, redis_basic
  database_basic  |
  database_basic  | PostgreSQL Database directory appears to contain a database ˜˜˜
  database_basic  |
  ˜˜˜
```
...
```
  ...
  bec89ece9546   postgres       ˜˜˜   Up 5 seconds   0.0.0.0:5432->5432/tcp   database_basic
  1f1efb10abbd   redis:alpine   ˜˜˜   Up 5 seconds   0.0.0.0:6379->6379/tcp   redis_basic
```

Para verificar o log de um container, em linha de comando digite o comando utilizando o nome do container desejado:

```
  docker logs database_basic
...
```
  docker exec -it database_basic /bin/bash
...
```
  ...
  [+] Running 3/3
   - Container database_basic  Removed                                  3.1s
   - Container redis_basic     Removed                                  3.2s
   - Network network_basic     Removed                                  0.2s
```
...
```
  ...
  [+] Running 3/3
   - Network network_basic     Started                                  0.0s
   - Container database_basic  Started                                  0.7s
   - Container redis_basic     Started                                  0.9s
```
